...
const VerticalNavbar = ({ activeSection, setActiveSection }) => {
  ...
  return (
    <div className={`bg-gradient-to-b from-slate-900 to-slate-800 shadow-2xl transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} min-h-screen flex flex-col fixed left-0 top-0 z-40 border-r border-slate-700`}>
      {/* Header */}
...
const StatCard = ({ title, value, change, changeType, icon: Icon, gradient, bgColor }) => (
  <div className={`bg-gradient-to-br ${gradient} p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-white/20`}>
    <div className="flex justify-between items-start mb-4">
      <div className="flex-1">
        <h3 className="text-sm font-semibold text-white/90 mb-2">{title}</h3>
        <p className="text-3xl font-bold text-white mb-1">{value}</p>
        <p className={`text-sm flex items-center ${changeType === 'up' ? 'text-green-200' : 'text-red-200'}`}>
    ...
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg font-bold text-gray-800">{Math.round(percentage)}%</p>
        </div>
      </div>
    </div>
  );
...
const Dashboard = () => {
  ...
  const [loading, setLoading] = useState(false);
   ...
   useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/health/summary`, {
  ...
  if (loading) {
    return (
      <div className="flex">
        <VerticalNavbar activeSection={activeSection} setActiveSection={setActiveSection} />
          ...
          <div className="p-6 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen">
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-slate-600 text-lg">Welcome back, {user?.name || 'User'}! Let's check your progress.</p>
              </div>
              <div className="flex items-center space-x-4">
                <div className="text-right bg-white/60 backdrop-blur-sm p-4 rounded-2xl border border-white/20">
                  <p className="text-sm font-semibold text-slate-700">Goal: {user?.goal || 'Not set'}</p>
                  <p className="text-sm text-slate-600">Age: {user?.age || 'Not set'}</p>
                </div>
            ...
            {tip && (
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6 mb-8 shadow-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-bold text-amber-800 text-lg">💡 AI Health Insight</p>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-8">
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex space-x-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-slate-600 font-medium">Sleep</span>
                    </div>
                    <div className="flex items-center space-x-2">
              ...
              {/* Enhanced Pie Chart */}
              <div className="bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
                <h3 className="text-xl font-bold text-slate-800 mb-6">Health Score Breakdown</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-slate-800">Sleep Quality</h3>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-slate-800">{summary.averageSleepHours}h</p>
              ...
              <div className="bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-slate-800">Hydration</h3>
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-slate-800">{summary.averageWaterIntake}L</p>
                  <p className="text-sm text-slate-600">Daily Average</p>
                </div>
              </div>
            ...
            <div className="bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
              <h3 className="text-xl font-bold text-slate-800 mb-6">Health Patterns & Insights</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gradient-to-r from-orange-50 to-red-50 p-4 rounded-xl border border-orange-200">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-bold text-orange-800">Most Frequent Meal</p>
                      <p className="text-orange-700 text-lg">{summary.mostFrequentMeal}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-green-50 to-teal-50 p-4 rounded-xl border border-green-200">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-bold text-green-800">Favorite Exercise</p>
                      <p className="text-green-700 text-lg">{summary.mostCommonExercise}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="p-6 bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
            ...
            <div className="bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/20">
              <p className="text-slate-600">
                Content for {activeSection.replace('-', ' ')} will be displayed here.
              </p>
            </div>
          </div>
        );
